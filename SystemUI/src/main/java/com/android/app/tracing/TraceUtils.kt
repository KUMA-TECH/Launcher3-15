...
inline fun <T> traceSection(tag: String, block: () -> T): T {
    val tracingEnabled = TraceCompat.isTagEnabled(TraceCompat.TRACE_TAG_APP)
    if (tracingEnabled) TraceCompat.traceBegin(TraceCompat.TRACE_TAG_APP, tag)
    return try {
        ...
        if (tracingEnabled) TraceCompat.traceEnd(TraceCompat.TRACE_TAG_APP)
    }
}
...
class TraceUtils {
    companion object {
        ...
        inline fun <T> traceAsync(trackName: String, method: String, block: () -> T): T {
            ...
            TraceCompat.asyncTraceForTrackBegin(TraceCompat.TRACE_TAG_APP, trackName, method, cookie)
            try {
                return block()
            } finally {
                TraceCompat.asyncTraceForTrackEnd(TraceCompat.TRACE_TAG_APP, trackName, cookie)
            }
        }
        ...
        suspend inline fun <T> traceCoroutine(
            ...
            val asyncTraceCookie =
                if (TraceCompat.isTagEnabled(TraceCompat.TRACE_TAG_APP))
                    Random.nextInt(FIRST_VALID_SPAN, Int.MAX_VALUE)
                else INVALID_SPAN
            if (asyncTraceCookie != INVALID_SPAN) {
                TraceCompat.asyncTraceForTrackBegin(
                    TraceCompat.TRACE_TAG_APP,
                    DEFAULT_TRACK_NAME,
                    spanName.value,
                    asyncTraceCookie
                )
            ...
            try {
                ...
                if (asyncTraceCookie != INVALID_SPAN) {
                    TraceCompat.asyncTraceForTrackEnd(
                        TraceCompat.TRACE_TAG_APP,
                        DEFAULT_TRACK_NAME,
                        asyncTraceCookie
                    )
        ...
        fun beginSlice(sliceName: String) {
            TraceCompat.traceBegin(TraceCompat.TRACE_TAG_APP, sliceName)
        }
         ...
         */
        fun endSlice() {
            TraceCompat.traceEnd(TraceCompat.TRACE_TAG_APP)
        }
        ...
        fun instant(eventName: String) {
            TraceCompat.instant(TraceCompat.TRACE_TAG_APP, eventName)
        }
        ...
        fun instantForTrack(trackName: String, eventName: String) {
            TraceCompat.instantForTrack(TraceCompat.TRACE_TAG_APP, trackName, eventName)
        }
    }
}
