...
import static com.android.launcher3.util.SystemApiCompatKt.getUserIdentifier;
...
import com.android.launcher3.util.SystemApiCompatKt;
...
public class SimpleIconCache extends BaseIconCache {
    ...
    protected long getSerialNumberForUser(@NonNull UserHandle user) {
        synchronized (mUserSerialMap) {
            int index = mUserSerialMap.indexOfKey(getUserIdentifier(user));
            if (index >= 0) {
                return mUserSerialMap.valueAt(index);
            }
            long serial = mUserManager.getSerialNumberForUser(user);
            mUserSerialMap.put(getUserIdentifier(user), serial);
            return serial;
        }
    }
    ...
    protected boolean isInstantApp(@NonNull ApplicationInfo info) {
        return SystemApiCompatKt.isInstantApp(info);
    }
